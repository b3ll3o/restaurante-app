  ...
  createClient: () => ({
    from: vi.fn((table: string) => {
      if (table === 'orders') {
        return {
          ...
          order: vi.fn().mockResolvedValue({ 
            data: [{ id: 'order-1', status: 'pending', total: 4590, created_at: new Date().toISOString() }], 
            error: null 
          }),
        };
      }
      if (table === 'categories') {
        return {
          select: vi.fn().mockReturnThis(),
          eq: vi.fn().mockResolvedValue({ data: [{ id: 'cat-1', name: 'Pizzas' }], error: null }),
        };
      }
      if (table === 'products') {
        return {
          select: vi.fn().mockReturnThis(),
          eq: vi.fn().mockResolvedValue({ data: [{ id: 'prod-1', name: 'Pizza Grande' }], error: null }),
        };
      }
      return {
        select: vi.fn().mockReturnThis(),
...
describe('DashboardPage - Testes de Integração', () => {
  ...
  describe('Cenário: Admin acessa dashboard', () => {
    it('deve renderizar conteúdo do dashboard', async () => {
    ...
    it('deve exibir métricas ou indicadores', async () => {
      const { default: DashboardPage } = await import('@/app/admin/dashboard/page');
