...
### Integração com PRD de Correção
...
1. RCA criado (.openspec/root-causes/RCA-YYYY-MM-DD-NNN.md)
       │
       ▼
2. Se mudança significativa → criar PRD de correção
   - Criar PRD em .openspec/backlog/prds/
   - PRD deve referenciar RCA ID (obrigatório)
       │
       ▼
3. PRD aprovado → seguir fluxo SDD completo
...
4. Após implementação:
   ...
   - RCA.Seção8 = "Correção Aplicada"
   - PRD movido para .openspec/backlog/archive/
   - RCA arquivado em .openspec/root-causes/
```
...
└─────────────────────────────────────────────────────────────────────────────┘
  ...
  └──────────────┘
         ...
         ▼
  ┌──────────────┐    ┌──────────────┐    ┌──────────────┐
  │ RCA Criado    │───▶│ PRD Criado   │───▶│ SDD Flow     │
  │ (obrigatório)│    │ (type=fix)  │    │ proposal→    │
  └──────────────┘    └──────────────┘    │ spec→design→ │
                                          │ tasks→impl→  │
                                          │ verify→      │
                                          │ archive      │
